function check_collisions_objects_prev_frame_vertical_mask_1(_solo_obj = -1) { // tp to where the bounding boxes just slightly touch. // only works if speed calculations are done in the step function!!
		...
		if(!_obstacle.ignore_collisions && _obstacle.z_level == z_level && _obstacle.object_index == _solo_obj) // WARNING: DOES NOT CHECK CHANGE IN Z LEVEL WHILE OVERLAPPING 
		{
