function check_collisions_objects_prev_frame_horizontal_exclusive(_ignoreobj = -1) { // tp to where the bounding boxes just slightly touch. // only works if speed calculations are done in the step function!!
		...
		for(var _j = 0;  _j < _n; _j++)
		{
			...
			if(!_horiz_obstacle.ignore_collisions && _horiz_obstacle.z_level == z_level && _horiz_obstacle.object_index != _ignoreobj) 
			{
