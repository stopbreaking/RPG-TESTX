function type_middlealign_hopping_letters_curve(x, y, _text, _progress, _width, _hop_progress, _hop_delay, _hop_height, _curve) {
	var _draw_x = 0;
	var _draw_y = 0;
	...
	for (var _i = 1; _i <= _progress; _i++;)
	{
		...
		{
			draw_text(x + _draw_x - _line_progress[_lines]*0.5, y + _draw_y - ((array_length(_line_progress) - 1)*string_height("A")/2) - _hop_height * animcurve_channel_evaluate(_cuv, clamp((_hop_progress), 0, 1)), _char);
			_hop_progress -= (_hop_delay);
			_draw_x += string_width(_char);
		}
	}
